...
# ---- Shiny App ----
ui <- fluidPage(
  titlePanel("Horizontprofil, Sonnenverlauf & Strahlung"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      dateInput("date", "Datum für Sonnenverlauf:", value = as.Date("2025-06-21")),
...
server <- function(input, output, session) {
  ...
  data_reactive <- eventReactive(input$go, {
    ...
    s <- sun_position(lat, lon, input$date, paste(interval, "min"))
  ...
  output$plot2 <- renderPlot({
    ...
    ggplot(s, aes(x = time, y = cum_radiation_day_MJ)) +
      ...
      labs(
        ...
        title = "Kumulative Tagesstrahlung"
      ) +
      theme_minimal(base_size = 14) +
      theme(plot.title = element_text(face = "bold", hjust = 0.5),
            axis.text.x = element_text(angle = 45, hjust = 1),
